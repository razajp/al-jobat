...
<div class="header w-full flex items-end justify-between">
    ...
    <div id="search-form" class="search-box w-1/3">
        ...
        <div class="search-input relative">
            ...
            <div class="dropdownMenu text-sm absolute mt-2 top-100 right-0 hidden border border-gray-600 w-48 bg-[var(--h-secondary-bg-color)] text-[var(--text-color)] shadow-lg rounded-xl opacity-0 transform scale-95 transition-all duration-300 ease-in-out z-50">
                <ul class="p-2">
                    @foreach ($filter_items as $key => $filter_item)
                        <li>
                            <label class="flex items-center justify-between cursor-pointer group py-2 px-3 hover:bg-[var(--h-bg-color)] grop rounded-md transition-all duration-200 ease-in-out" onclick='setFilter("{{ $key }}")'>
                                <input type="radio" name="filter" value="{{ $key }}" class="hidden peer" {{ $key == 'all' ? 'checked' : '' }}/>
                                <span class="transition-all peer-checked:text-[var(--primary-color)] peer-checked:ml-1 grop-hover:text-[var(--primary-color)]">{{ $filter_item }}</span>
