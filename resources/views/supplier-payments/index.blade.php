@extends('app')
@section('title', 'Show Supplier Payments | ' . app('company')->name)
@section('content')
    <!-- Modals -->
    <div id="modal"
    ...
    <div class="w-[80%] mx-auto">
        <x-search-header heading="Supplier Payments" :filter_items="[
            'all' => 'All',
    ...
    <section class="text-center mx-auto ">
        <div
            ...
            <x-form-title-bar title="Show Supplier Payments" changeLayoutBtn layout="{{ $authLayout }}" />

            @if (count($payments) > 0)
                <div class="absolute bottom-3 right-3 flex items-center gap-2 w-fll z-50">
                    <x-section-navigation-button link="{{ route('supplier-payments.create') }}" title="Add New Payment" icon="fa-plus" />
                </div>
                                ...
                                <div class="search_container grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                                    @foreach ($payments as $payment)
                                        <div id="{{ $payment->id }}" data-json='{{ $payment }}'
                                            ...
                                            <x-card :data="[
                                                'name' => 'Supplier: ' . $payment->supplier->supplier_name,
                                                'details' => [
                                ...
                                <div class="search_container overflow-y-auto grow my-scrollbar-2">
                                    @forEach ($payments as $payment)
                                        <div id="{{ $payment->id }}" data-json='{{ $payment }}' class="contextMenuToggle modalToggle relative group grid text- grid-cols-4 border-b border-[var(--h-bg-color)] items-center py-2 cursor-pointer hover:bg-[var(--h-secondary-bg-color)] transition-all fade-in ease-in-out">
                                            <span class="text-center">{{ $payment->supplier->supplier_name }}</span>
                ...
                <x-modal id="modalForm" closeAction="closeModal">
                    ...
                    <div class="flex items-start relative h-[15rem]">
                        <div class="flex-1 h-full overflow-y-auto my-scrollbar-2">
                            <div class="px-2">
                                <h5 id="name" class="text-2xl mb-2 text-[var(--text-color)] capitalize font-semibold leading-none">Supplier: ${data.supplier.supplier_name}</h5>
