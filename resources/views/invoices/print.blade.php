    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            invoices.forEach((invoice) => {
                ...
                let customerData = invoice.customer; // Added 'let' declaration
                let totalPcs = 0;
                let totalPackets = 0;
                let totalAmount = 0;
                    ...
                    previewDom.innerHTML = `
                        <div class="preview-container w-[210mm] h-[302mm] mx-auto overflow-hidden relative">
                            ${articlePages.map((page, pageIndex) => {
                                // Reset totals for each page calculation or calculate per page
                                let pageTotalPcs = 0;
                                let pageTotalPackets = 0;
                                                ...
                                                <div class="invoice-table w-full">
                                                    <div class="table w-full border border-black rounded-lg pb-2.5 overflow-hidden">
                                                        ...
                                                        <div id="tbody" class="tbody w-full">
                                                            ${page.map((fetchedArticle, articleIndex) => {
                                                                let article = fetchedArticle.article;
                                                                const hrClass = articleIndex === 0 ? "mb-2.5" : "my-2.5";
                                                                let quantity = fetchedArticle.shipment_pcs * cottonCount;
                                                                let packets = Math.floor(quantity / article.pcs_per_packet);
                                                                ...
                                                                // Calculate the actual serial number across pages
