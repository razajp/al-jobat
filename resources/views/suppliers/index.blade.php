                    ...
                    <div class="card_container grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                        @foreach ($Suppliers as $supplier)
                            <div data-json='{{ $supplier }}'
                                ...
                                <x-card :data="[
                                    'image' => $supplier->user['profile_picture'] == 'default_avatar.png' ? asset('images/default_avatar.png') : asset('storage/uploads/images/' . $supplier->user['profile_picture']),
        ...
        <div class="context-menu absolute top-0 text-sm" style="display: none;">
            <div
                ...
                <ul class="p-2">
                    ...
                    <li>
                        <button id="show-details" type="button"
                            class="flex items-center w-full px-4 py-2 text-left hover:bg-[--h-bg-color] rounded-md transition-all 0.3s ease-in-out">Manage Category</button>
                    </li>
    ...
    <script>
        ...
        function generateModal(item) {
                ...
                <x-modal id="modalForm" closeAction="closeModal" action="{{ route('update-user-status') }}">
                    ...
                    <div class="flex items-start relative h-[15rem]">
                        ...
                        <div class="flex-1 ml-8 h-full overflow-y-auto my-scroller-2">
                            ...
                            <p class="text-[--secondary-text] mb-1 tracking-wide text-sm"><strong>Person Name:</strong> <span id="person_name" class="person_name">${data.person_name}</span></p>
                            <p class="text-[--secondary-text] mb-1 tracking-wide text-sm"><strong>Username:</strong> <span id="username" class="username">${data.user.username}</span></p>
                            <p class="text-[--secondary-text] mb-1 tracking-wide text-sm"><strong>Phone Number:</strong> <span id="phone_number" class="phone_number">${data.phone_number}</span></p>
            ...
            if (data.user.profile_picture == "default_avatar.png") {
                ...
                userImage.src = `storage/uploads/images/${data.user.profile_picture}`
            }
        ...
        function generateManageCategoryModal() {
            manageCategoryModalDom.innerHTML = `
                <x-modal id="manageCategoryForm" closeAction="closeManageCategoryModal" action="#">
                    <!-- Modal Content Slot -->
                    <div id="active_inactive_dot_modal"
                        class="absolute top-3 left-3 w-[0.7rem] h-[0.7rem] bg-[--border-success] rounded-full">
                    </div>
                    ...
                    <x-slot name="actions">
                        ...
                        <div id="ac_in_modal">
                            ...
                            <input type="hidden" id="user_status" name="status" value="active">
                            <button id="ac_in_btn" type="submit"
                                class="px-4 py-2 bg-[--bg-error] border border-[--bg-error] text-[--text-error] font-semibold rounded-lg hover:bg-[--h-bg-error] transition-all duration-300 ease-in-out">
                                In Active
                            </button>
